...
from ROSA_modules import rasa_chatbot_module
...
def main():
    ...
    last_username = ""
    guid = rasa_chatbot_module.conversation_id_generator()
    ...
    while(True):
        ...
        if text != "":
            # Chatbot
            guid = rasa_chatbot_module.conversation_id_verifier(username, last_username, guid)
            intent, response = rasa_chatbot_module.api_chat(text, guid)

            # Defining emotion for ROSA
            eyes_shape = face_module.choose_emotion(intent)
            ...
            face_module.speak_feedback(feedback, feedback_color, feedback_color,feedback_shape)
            face_module.write_text(response+" "+username, turtle_text, text_color, text_fill_color, 24, -700, -400)
            speak_module.speak(response+username)
            face_module.reset_turtle(turtle_text)

            # Returning to neutral emotion
            eyes_shape = 'ellipse'
            face_module.reset_turtle(right_eye)
            face_module.reset_turtle(left_eye)
            face_module.full_face_draw(screen, face, right_eye, left_eye, mouth, bg, face_color, eyes_color, mouth_color, bg_color, face_shape, eyes_shape, mouth_shape, face_size, eyes_size, mouth_size, face_is_filled, eyes_is_filled, mouth_is_filled, face_fill_color, eyes_fill_color, mouth_fill_color, has_face, has_eyes, has_mouth)

        # Updating the last username
